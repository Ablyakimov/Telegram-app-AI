...
export class ChatsController {
  ...
  async create(@Body() createChatDto: CreateChatDto, @Req() req: Request) {
    ...
    if (telegramUser) {
      ...
      const accessCheck = await this.subscriptionService.checkAccess(
        dbUser.id,
        model,
      );
  ...
  async sendMessage(
    ...
    const { chatId, message, systemPrompt, temperature, maxTokens } =
      sendMessageDto;
    const telegramUser = req["telegramUser"];
    ...
    if (this.dedupCache.isDuplicate(telegramUser.id, chatId, message)) {
      throw new BadRequestException({
        message:
          "Duplicate message detected. Please wait a moment before sending the same message again.",
    ...
    if (!accessCheck.allowed) {
      ...
      if (accessCheck.reason === "subscription_expired") {
        ...
        errorMessage =
          "This model is not available on your current plan. Please upgrade to PRO.";
      } else if (accessCheck.reason === "monthly_limit_reached") {
        errorMessage =
          "Monthly message limit reached. Please buy credits or upgrade to PRO.";
      }
    ...
    const updatedChat = await this.chatsService.addMessage(
      chatId,
      "user",
      message,
    );
    ...
    await this.subscriptionService.deductUsage(
      telegramUser.id,
      chat.aiModel,
      0,
    );
  ...
  async uploadFile(
    ...
    try {
      if (
        ...
        const aiResponse = await this.aiService.chat(
          [...chat.messages, { role: "user", content: contextMessage }],
          { model: chat.aiModel },
        );
        ...
        try {
          ...
          if (openai?.audio?.transcriptions) {
            ...
            extractedText =
              "[Audio uploaded but transcription service not available]";
          }
        } catch (e) {
