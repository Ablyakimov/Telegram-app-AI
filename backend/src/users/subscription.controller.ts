...
export class SubscriptionController {
  ...
  async buyCredits(@Body() buyCreditsDto: BuyCreditsDto, @Req() req: Request) {
    ...
    return this.subscriptionService.addCredits(telegramUser.id, buyCreditsDto.amount);
  }

  @Post('upgrade')
  async upgradePlan(@Body() upgradePlanDto: UpgradePlanDto, @Req() req: Request) {
    const telegramUser = req['telegramUser'];
  ...
  async getPlanLimits() {
    return {
      ...
      [PlanType.ENTERPRISE]: this.subscriptionService.getPlanLimits(PlanType.ENTERPRISE),
    };
  }
